...
/*
# 메모리 크기와 정수의 범위 (1바이트 = 8비트)
1 비트: 2**1 (칸이 1개라서 경우의 수 2)
  - 음수가 없는 경우: 0, 1
2 비트: 2**2 (칸이 2개라서 경우의 수 4)
  - 음수가 없는 경우: 00(0), 01(1), 10(2), 11(3) (0 ~ 3)
  - 음수가 있는 경우: 00(0), 01(1), 10(-2), 11(-1) (-2 ~ 1)
4 비트: 2**4 (칸이 4개라서 2*2*2*2 경우의 수 16)
  - 음수가 없는 경우: 0 ~ 2**4 - 1 = 0 ~ 15 
  - 음수가 있는 경우: -2**3 ~ 2**3 - 1 = -8 ~ 7
8 비트: 2**8 (칸이 8개라서 2의 8승 경우의 수 256 = 1바이트)
  - 음수가 없는 경우: 2**8 = 0 ~ 2**8-1 = 0 ~ 255 
  - 음수가 있는 경우: -2**7 ~ 2**7-1 = -128 ~ 127
16비트: 2**16 (칸이 16개라서 2의 16승 경우의 수 65536 = 2바이트)
  - 음수가 없는 경우: 0 ~ 2**16-1 = 0 ~ 65535
  - 음수가 있는 경우: -2**15 ~ 2**15-1 = -32768 ~ 32767
32비트: 2^32 (칸이 32개라서 2의 32승 경우의 수 4,294,967,296 = 4바이트)
  - 음수가 없는 경우: 0 ~ 2**32 - 1 = 4,294,967,295
  - 음수가 있는 경우: -2**31 ~2**31 - 1 = -2,147,483,648 ~ 2,147,483,647
64비트: 2^64 = (칸이 64개라서 2의 64승 = 6바이트)
