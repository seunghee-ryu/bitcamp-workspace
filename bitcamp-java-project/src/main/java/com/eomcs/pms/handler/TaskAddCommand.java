...
public class TaskAddCommand implements Command {
  ...
  public TaskAddCommand(TaskService taskService, ProjectService projectService, MemberService memberService) {
  ...
  public void execute(Map<String,Object> context) {
    ...
    // 프로젝트 목록을 가져온다.
    try {
      List<Project> projects = projectService.list(keyword);
      if (projects.size() == 0) {
      ...
      List<Member> members = memberService.findByProjectNo(task.getProjectNo());
      if (members.size() == 0) {
